...
scalability. This work extends our previous conference study and presents
RALA-ChangeFormer, an efficient variant
...
by $50\%$ at $256{\times}256$ resolution. This journal extension evaluates both
the baseline and the proposed model, trained under an identical protocol,
across four additional benchmarks spanning diverse acquisition conditions:
...
heterogeneity: the model generalizes reliably to nadir-view datasets with
...
to accuracy-maximization strategies, and motivate future work on geometric
...
accuracy and the most precise building annotations. Approximately 32\,000 bi-temporal
...
building-shadow variability, and apparent positional shifts of rooftop
boundaries between temporal acquisitions; these conditions differ substantially
from the nadir-view imagery on which the evaluated models were pretrained.
...
LEVIR-CD checkpoints with an initial learning rate of $6\times10^{-5}$ under
cosine annealing. Binary cross-entropy combined with a soft Dice loss was used
...
experiments were executed on an NVIDIA A100 40\,GB GPU.}
...
\cite{Bandara2022ChangeFormer}; CF and RALA-CF results are from the conference
version of this work \cite{acuna2026efficient}. The best value per dataset is
shown in bold.}
\label{tab:accuracy}
\centering
\resizebox{\columnwidth}{!}{
...
CF~\cite{Bandara2022ChangeFormer} & LEVIR-CD & \textbf{90.40} & \textbf{82.48} & \textbf{99.04} \\
\rowcolor{pastelorange}
RALA-CF (ours) \cite{acuna2026efficient} & LEVIR-CD & 89.87 & 81.60 & 98.99 \\

\hline
SNUNet-CD~\cite{Fang2021SNUNetCD} & DSIFN-CD & 66.18 & 49.45 & 87.34 \\
BIT~\cite{Chen2022BIT}            & DSIFN-CD & 69.26 & 52.97 & 89.41 \\
CF~\cite{Bandara2022ChangeFormer} & DSIFN-CD & \textbf{86.67} & \textbf{76.48} & \textbf{95.56} \\
\rowcolor{pastelorange}
RALA-CF (ours) \cite{acuna2026efficient} & DSIFN-CD & 84.07 & 73.96 & 92.44 \\
\hline
\end{tabular}
}
\end{table}
...
Table~\ref{tab:master} reports the full metric suite for both the ChangeFormer
baseline and RALA-ChangeFormer across the four extended benchmarks; every entry
originates from experiments executed by the authors under the identical
training protocol.
...
WHU-CD yields the strongest result for both models, and RALA-ChangeFormer
attains the best scores there ($F_1=87.85\%$ vs.\ $87.49\%$ for the baseline,
$\mathrm{IoU}=78.33\%$ vs.\ $77.77\%$), consistent with the clean annotations
and nadir-view geometry of this dataset.
On LEVIR-CD+ the baseline retains a marginal advantage
($F_1=80.34\%$ vs.\ $80.24\%$), while RALA-ChangeFormer achieves higher recall
...
morphologies.
SYSU-CD shows the same pattern ($F_1=78.05\%$ for the baseline vs.\ $78.02\%$
for RALA-ChangeFormer) despite its multi-class change complexity, suggesting
sufficient discriminative capacity in the rank-augmented encoder.
S2Looking yields the lowest scores of the four benchmarks for both models,
with RALA-ChangeFormer obtaining the better values across every metric
($F_1=60.39\%$ vs.\ $59.88\%$); the low recall of both models
(near $53\%$) indicates that the side-looking perspective distortion
constitutes an acquisition condition that neither attention
formulation overcomes.
}

\begin{table}[h!]
\caption{\journal{Comparison between the ChangeFormer baseline and the proposed
RALA-ChangeFormer across the four extended benchmarks (LEVIR-CD+, SYSU-CD,
WHU-CD, and S2Looking). Both models were trained and evaluated by the authors
under the identical protocol described in Section~\ref{sec:experiments}.
All values are reported in \%; the best value per metric and dataset is shown
in bold.}}
\label{tab:master}
\centering
\resizebox{\columnwidth}{!}{
...
& \textbf{Prec} & \textbf{Rec} & \textbf{F1} & \textbf{IoU} & \textbf{OA} \\
\hline
LEVIR-CD+ & \textbf{81.96} & 78.79 & \textbf{80.34} & \textbf{67.14} & \textbf{98.43} &
80.20 & \textbf{80.29} & 80.24 & 67.01 & 98.39 \\

SYSU-CD & \textbf{83.57} & 73.21 & \textbf{78.05} & \textbf{64.00} & \textbf{90.29} &
82.10 & \textbf{74.34} & 78.02 & 63.97 & 90.12 \\

WHU-CD & \textbf{90.97} & 84.27 & 87.49 & 77.77 & 99.04 &
90.14 & \textbf{85.67} & \textbf{87.85} & \textbf{78.33} & \textbf{99.06} \\

S2Looking & 68.81 & 53.00 & 59.88 & 42.73 & 99.14 &
\textbf{70.02} & \textbf{53.09} & \textbf{60.39} & \textbf{43.25} & \textbf{99.16} \\
\hline
\end{tabular}
}
\end{table}
...
The six-dataset results (Table~\ref{tab:accuracy} and Table~\ref{tab:master})
...
Both models obtain their highest scores on WHU-CD ($F_1=87.85\%$ for RALA-CF
...
The scores on LEVIR-CD+ ($F_1=80.24\%$ for RALA-CF and $80.34\%$ for CF) align
...
The results on SYSU-CD ($F_1=78.02\%$ for RALA-CF and $78.05\%$ for CF) are
...
Both models obtain their lowest scores on S2Looking ($F_1=60.39\%$ for RALA-CF
...
between quadratic and rank-augmented linear attention. This observation frames
